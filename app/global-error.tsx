...
async function sendFrontendError(error: Error & { digest?: string }) {
  try {
...
export default function GlobalError({
  ...
  useEffect(() => {
    void sendFrontendError(error);
  }, [error]);

  return (
    <html lang="en">
      <body className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(45,212,191,0.16),transparent_30%),linear-gradient(180deg,#f8fafc_0%,#eef5ff_100%)] px-6 py-10 text-slate-950">
        <div className="mx-auto flex min-h-[70vh] w-full max-w-3xl items-center justify-center">
          <div className="w-full rounded-4xl border border-white/80 bg-white/90 p-8 text-center shadow-[0_24px_70px_rgba(15,23,42,0.12)] backdrop-blur-xl">
            ...
            <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-950">JobMate hit an unexpected issue.</h1>
